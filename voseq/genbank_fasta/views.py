...
def results(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            cleaned_data = fix_cleaned_data(form)
            ...
            context['errors'] = dataset_creator.errors
            context['protein_file'] = aa_dataset_file
            context['warnings'] = dataset_creator.warnings
        context["form"] = form
        return render(request, 'genbank_fasta/index.html', context)
...
def serve_file(request, file_name):
    ...
    fasta_file = os.path.join(cwd, '..', 'create_dataset', 'dataset_files', file_name)
